...
// resolving.
function endAfterEventLoopTurn(test) {
  return new Promise(resolve => test.step_timeout(resolve, 0));
}
...
function encodeOrDecodeShouldThrow(codec, input) {
  ...
  if ("decode" in codec.__proto__) {
    assert_throws_dom("InvalidStateError",
                      () => codec.decode(input),
                      "decode");
  } else if ("encode" in codec.__proto__) {
